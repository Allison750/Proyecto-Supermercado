...
public class BienvenidoCajero extends JFrame implements ActionListener {
  ...
  ArregloLogin alc=new ArregloLogin();
	...
	private JComboBox comboBoxDíasAuse;
	...
	public BienvenidoCajero() {
		...
		comboBoxDíasAuse = new JComboBox();
		comboBoxDíasAuse.setModel(new DefaultComboBoxModel(new String[] {"Seleccione la cantidad de d\u00EDas que estuvo ausente en el mes", "0", "1", "2", "3"}));
		comboBoxDíasAuse.setBounds(487, 167, 94, 20);
		contentPane.add(comboBoxDíasAuse);
		...
		modelo.addColumn("Días Asistidos");
		modelo.addColumn("Días Ausentes");
						...
						try{
							...
							if(cargoCaj.length() > 0){
								try{
									...
									if(tipoCaj.length() > 0){
										try{
											int da=Integer.parseInt(Días_asistidos()); 
											try{
												int df=Integer.parseInt(Días_falta()); 
												try{
															...
															try{
																...
																if(contraseña_registrada.length() > 0){
																	...
																	try{
																		 ...
																		 pw.print(caj.getDías_asistidos()+"\t");
																		 pw.print(caj.getDías_falta()+"\t");
	...
	void Listar(){
		...
		for(int i=0; i<ac.Tamaño(); i++){
			...
			ac.Obtener(i).getDías_asistidos(),
			ac.Obtener(i).getDías_falta(),
	...
	String Días_asistidos(){
		return comboBoxDiasAsis.getSelectedItem().toString();
	}
	String Días_falta(){
		return comboBoxDíasAuse.getSelectedItem().toString();
	}
	...
	protected void do_btnBuscar_actionPerformed(ActionEvent arg0) {
		...
		try{
			...
			if(x != null){
				Mensaje("Se encontró "+ID_Empleado()+" "+"exitosamente");
				...
				comboBoxDiasAsis.setSelectedItem(""+x.getDías_asistidos());
				comboBoxDíasAuse.setSelectedItem(""+x.getDías_falta());
				txtTarifaDiariaCaj.setText(""+x.getTarifa_diaria());
				txtUsuarioCajeroBienve.setText(""+x.getLogcaj().getUsuario());
				passwordCajero.setText(""+x.getLogcaj().getContraseña());
			}else{
	...
	protected void do_btnEliminar_actionPerformed(ActionEvent arg0) {
		try {
			...
			if (x != null) {
				ac.Eliminar(x);
				Mensaje("Se eliminó "+ID_Empleado()+" "+"exitosamente");
				//Listar();
				txtIdCajero.setText("");
				...
				comboBoxDíasAuse.setSelectedItem(" ");
				...
				if(nombre.length()>0){
					try{
						...
						try{
							...
							x.setDías_asistidos(Integer.parseInt(Días_asistidos()));
							x.setDías_falta(Integer.parseInt(Días_falta()));
	...
	protected void do_btnListar_actionPerformed(ActionEvent arg0) {
		String usuario,contraseña,idc,nom,appe,car,tc,da,dau,td;
		...
		File doc_1e=new File(au);
				try{
					...
					while(linea.hasNextLine()){
						...
						contraseña=linea.nextLine();

						modelo.addRow(new Object[]{idc,nom,appe,car,tc,da,dau,td,usuario,contraseña});
					}
				}catch(Exception x){
